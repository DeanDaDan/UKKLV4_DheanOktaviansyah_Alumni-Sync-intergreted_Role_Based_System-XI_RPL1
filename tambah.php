...
$errors = [];
$old    = ['nim' => '', 'angkatan' => '', 'nama' => '', 'jurusan' => '', 'email' => '', 'alamat' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi dengan struktur pola baru (associative error keys)
    if ($nim === '') {
        $errors['nim'] = 'NIS wajib diisi.';
    } else {
        $chk = $pdo->prepare("SELECT id_alumni FROM alumni WHERE nim = ?");
        $chk->execute([$nim]);
        if ($chk->fetch()) {
            $errors['nim'] = 'NIS sudah terdaftar.';
        }
    }
    ...
    $old = compact('nim', 'angkatan', 'nama', 'jurusan', 'email', 'alamat');

    if (empty($errors)) {
        $stmt = $pdo->prepare("
            INSERT INTO alumni (nim, nama, angkatan, jurusan, email, alamat)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([$nim, $nama, $angkatan, $jurusan, $email, $alamat ?: null]);

        $_SESSION['flash'] = [
            'type' => 'success',
            'msg'  => "Data alumni \"$nama\" berhasil ditambahkan."
        ];
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'DM Sans', sans-serif; }
        ...
        /* GAYA ERROR SPAN (DIADOPSI DARI POLA BARU) */
        .form-error { color: #d9534f; font-size: 0.85rem; margin-top: 4px; }
    </style>
</head>
<body>
            ...
            <form method="POST" autocomplete="off">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        <label>NIS *</label>
                        <input type="text" name="nim" class="form-control" placeholder="Contoh: 5532xxxxx" value="<?= htmlspecialchars($old['nim']) ?>" required>
                        <?php if (isset($errors['nim'])): ?><span class="form-error"><?= $errors['nim'] ?></span><?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label>Angkatan *</label>
                        <input type="number" name="angkatan" class="form-control" placeholder="Contoh: 11" value="<?= htmlspecialchars($old['angkatan']) ?>" required>
                        <?php if (isset($errors['angkatan'])): ?><span class="form-error"><?= $errors['angkatan'] ?></span><?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label>Nama Lengkap *</label>
                        <input type="text" name="nama" class="form-control" placeholder="Contoh: Budi Santoso" value="<?= htmlspecialchars($old['nama']) ?>" required>
                        <?php if (isset($errors['nama'])): ?><span class="form-error"><?= $errors['nama'] ?></span><?php endif; ?>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="jurusan" class="form-control" required>
                            ...
                            <option value="Teknik Jaringan Akses Telekomunikasi" <?= $old['jurusan'] === 'Teknik Jaringan Akses Telekomunikasi' ? 'selected' : '' ?>>Teknik Jaringan Akses Telekomunikasi</option>
                            <option value="Animasi" <?= $old['jurusan'] === 'Animasi' ? 'selected' : '' ?>>Animasi</option>
                        </select>
                        <?php if (isset($errors['jurusan'])): ?><span class="form-error"><?= $errors['jurusan'] ?></span><?php endif; ?>
                    </div>
                </div>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full">
                        ...
                        <textarea name="alamat" class="form-control" rows="3" placeholder="Alamat lengkap saat ini"><?= htmlspecialchars($old['alamat']) ?></textarea>
                        <?php if (isset($errors['alamat'])): ?><span class="form-error"><?= $errors['alamat'] ?></span><?php endif; ?>
                    </div>
                </div>
